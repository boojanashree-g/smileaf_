$(document).ready(function () {
  var mode, JSON, res_DATA, shape_id;

  $.when(getShapeDetails()).done(function () {
    dispShapeDetails(JSON);
  });
  ...
  $("#btn-submit").click(function () {
    $(".error").hide();
    if ($("#shape_name").val() === "" && mode == "new") {
      $(".shape_name").html("Please Enter name*").show();
    } else {
      insertData();
    }
  });
  ...
  function insertData() {
    ...
    var url;
    if (mode == "new") {
      url = base_Url + "admin/filter-shapes/insert-data";
    } else if (mode == "edit") {
      url = base_Url + "admin/filter-shapes/update-data";
      data.append("shape_id", shape_id);
    }
  ...
  function dispShapeDetails(JSON) {
    var i = 1;
  ...
  $("#submit-status").click(function () {
    ...
    if (status === "") {
      ...
      $.ajax({
        type: "POST",
        url: base_Url + "admin/filter-shapes/update-status",
  ...
  $(document).on("click", ".btnEdit", function () {
    ...
    $("#shape_name").val(res_DATA[index].shape_name);

    shape_id = res_DATA[index].shape_id;
  });
  ...
  $(document).on("click", ".BtnDelete", function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: "POST",
          url: base_Url + "admin/filter-shapes/delete-data",
